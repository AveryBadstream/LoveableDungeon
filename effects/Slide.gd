...
func prep():
	if WRLD.cell_occupied(steps[step]):
		EFCT.queue_now(slam_effect.new(effect_actor, effect_target, (steps[step-1] - steps[step]).normalized(), steps[step-1], magnitude))
		return false
	elif steps.size() > step + 1:
		EFCT.queue_at_offset(get_script().new(effect_actor, effect_target, steps, step + 1, magnitude - 1), 1)
	else:
		EFCT.queue_at_offset(slam_effect.new(effect_actor, effect_target, (steps[step-1] - steps[step]).normalized(), steps[step], magnitude - 1), 1)	
